...
/*
  ...
  sindex_t send_size = 0;
  sindex_t send_off = 0;

  for(ii = 0; ii < mpi_size; ii ++){
    index_t send_n = seed_kinds / mpi_size;
    if(ii == mpi_size - 1){
      send_n = seed_kinds - ii * (seed_kinds/mpi_size);
    }
    ...
    MPI_Gather(&(seed_count[ii * (seed_kinds / mpi_size)]), send_n, MPI_UINT32_T,
        seed_all_count, send_n, MPI_UINT32_T, ii, MPI_COMM_WORLD); 
    ...
    send_off =  seeds_mpi_off[ii];
    send_size = seeds_mpi[ii]; 

    sindex_t *send_buf ;
    send_buf = positions + send_off;

    half_word_t *lseeds_send;
    lseeds_send = pos_lseeds + send_off;


#ifdef MPE_PROFILE
    GPTLstart("MPIcomm");
#endif
    MPI_Gatherv(send_buf, send_size, MPI_UINT32_T, recive_pos, recive_size, disp, MPI_UINT32_T,ii, MPI_COMM_WORLD);
    MPI_Gatherv(lseeds_send, send_size, MPI_UINT16_T, recive_lseeds, recive_size, disp, MPI_UINT16_T,ii, MPI_COMM_WORLD);
